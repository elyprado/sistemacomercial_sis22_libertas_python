...
function alterar(id) {
    ...
    fetch("http://127.0.0.1:5000/cidade/" + id)
    .then(resp => resp.json())
    .then(dados => {
        //preenche os inputs
        const txtnomecidade = document.getElementById("txtnomecidade");
...
function listar() {
    ...
    fetch("http://127.0.0.1:5000/cidade?pesquisa=" + txtpesquisa.value)
    .then(resp => resp.json())
...
function excluirSim() {
    fetch("http://127.0.0.1:5000/cidade/" + idatual,
        {
            headers: {
...
function salvar() {
    ...
    if (idatual<=0) {
        ...
        //alterar
        url = "http://127.0.0.1:5000/cidade/" + idatual;
        metodo = "PUT";
    }
    fetch(url,
        {
